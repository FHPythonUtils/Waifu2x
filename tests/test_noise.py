...
from waifu2x import denoise_image, load_models
...
def test_foreground_png():
	...
	dst = src.copy()
	dst = denoise_image(args, dst, models["noise"])
	# dst.convert(src.mode).save(f"{THISDIR}/data/foreground_noise_expected.png")
	assert imgcompare.is_equal(
		dst,
		f"{THISDIR}/data/foreground_noise_expected.png",
		tolerance=0.2,
	)
